...
namespace Ordenar
{
    internal class FormatChunk
    {
        ...
        {
            return $"{{{nameof(ChunkId)}={ChunkId}, {nameof(ChunkSize)}={ChunkSize.ToString()}, {nameof(FormatTag)}={FormatTag.ToString()}, {nameof(Channels)}={Channels.ToString()}, {nameof(Frequency)}={Frequency.ToString()}, {nameof(AverageBytesPerSec)}={AverageBytesPerSec.ToString()}, {nameof(BlockAlign)}={BlockAlign.ToString()}, {nameof(BitsPerSample)}={BitsPerSample.ToString()}}}";
        }
    }
}
